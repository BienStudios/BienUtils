 ...
 * sanitizado, permitiendo únicamente números y letras (sin acentuación);
 ...
 * @example
 * input.addEventListener("blur", validateAlphaNumeric);
 * // Para `keydown`, sólo se activará con la tecla "Enter"
 * input.addEventListener("keydown", validateAlphaNumeric);
 */
export default async function validateAlphaNumeric(e) {
    if (e.type === "keydown") {
        if (e.key !== "Enter") return;
    }
    let rs;
    try {
        rs = await helperSentinel(e.target.value, "alnum", "HTML");
    } catch (e) {
